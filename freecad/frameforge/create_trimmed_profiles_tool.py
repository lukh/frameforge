...
from freecad.frameforge.trimmed_profile import (TrimmedProfile,
                                                ViewProviderTrimmedProfile)
...
class CreateTrimmedProfileTaskPanel:
    ...
    def initialize_ui(self):
        ...
        self.form.rb_simplecut.toggled.connect(
            lambda: self.update_cuttype("Simple cut")
        )
    ...
    def set_trimmed_body(self):
        if len(Gui.Selection.getSelectionEx()) == 1:
            ...
            App.Console.PrintMessage(
                translate("frameforge", f"Set Trimmed body: {trimmed_body.Name}\n")
            )
    ...
    def add_trimming_bodies(self):
        ...
        for selObject in Gui.Selection.getSelectionEx():
            if all(
                [
                    tb != (selObject.Object, tuple(selObject.SubElementNames))
                    for tb in trimming_boundaries
                ]
            ):
                trimming_boundaries.append(
                    (selObject.Object, tuple(selObject.SubElementNames))
                )
                ...
                App.Console.PrintMessage(
                    translate(
                        "frameforge", "Already a trimming body for this TrimmedBody\n"
                    )
                )
    ...
    def remove_trimming_bodies(self):
        ...
        selected_tb = [
            item.data(1) for item in self.form.boundaries_list_widget.selectedItems()
        ]
        self.fp.TrimmingBoundary = [
            tb for tb in self.fp.TrimmingBoundary if tb not in selected_tb
        ]
    ...
    def update_view_and_model(self):
        ...
        for bound in self.fp.TrimmingBoundary:
            ...
            item.setText(
                "{} ({} {})".format(bound[0].Label, bound[0].Name, ", ".join(bound[1]))
            )
    ...
    def open(self):
        App.Console.PrintMessage(
            translate("frameforge", "Opening Create Trimmed Profile\n")
        )
        App.ActiveDocument.openTransaction("Update Trim")

    def reject(self):
        App.Console.PrintMessage(
            translate("frameforge", f"Rejecting CreateProfile {self.mode}\n")
        )
    ...
    def accept(self):
        App.Console.PrintMessage(
            translate("frameforge", "Accepting Create Trimmed Profile\n")
        )
...
class TrimProfileCommand:
    ...
    def Activated(self):
        ...
        if len(sel) == 0:
            ...
            trimmed_profile = self.make_trimmed_profile(
                trimmedBody=sel[0].Object, trimmingBoundary=trimmingboundary
            )
        App.ActiveDocument.commitTransaction()
