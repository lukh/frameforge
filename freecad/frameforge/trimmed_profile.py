...
class TrimmedProfile:
    def __init__(self, obj):
        obj.addProperty(
            ...
            "TrimmedBody",
            "TrimmedProfile",
            translate("App::Property", "Body to be trimmed"),
        ).TrimmedBody = None
        obj.addProperty(
        ...
        obj.addProperty(
            ...
            "CutType",
            "TrimmedProfile",
            translate("App::Property", "Cut Type"),
        ).CutType = [
            "Coped cut",
            "Simple cut",
        ]
    ...
    def execute(self, fp):
        ...
        if fp.TrimmedProfileType == "End Trim":
            if fp.CutType == "Coped cut":
                ...
                shps = BOPTools.SplitAPI.slice(
                    fp.TrimmedBody.Shape, shapes, mode="Split"
                )
            ...
            for target in bounds_target:
                trimming_boundary_edges.append(
                    doc.getObject(target[0].Name).getSubObject(target[1][0])
                )
            for edge2 in trimming_boundary_edges:
                ...
                if (
                    end1.distanceToPoint(start2) < precision
